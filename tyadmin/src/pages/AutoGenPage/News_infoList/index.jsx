...
import { addNews_info, queryNews_info, removeNews_info, updateNews_info, queryNews_infoVerboseName, queryNews_infoListDisplay, queryNews_infoDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateNews_info(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryNews_infoDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryNews_infoListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryNews_info, table_columns, 'News-All')}>
            <ExportOutlined /> Export all
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="Search News" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'News-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryNews_info({ ...params, sorter, filter })}
