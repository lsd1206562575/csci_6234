...
import { addDoctor_visit, queryDoctor_visit, removeDoctor_visit, updateDoctor_visit, queryDoctor_visitVerboseName, queryDoctor_visitListDisplay, queryDoctor_visitDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateDoctor_visit(value, current_id);
      hide();
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
                        ...
                        },
                      {
                             ...
                             dataIndex: 'doctor_visit',
                             
                             rules: [
                                     {
                      required: true,
                      message: 'doctor_visit is required',
                     },
                             ],
          ...
          width: 100,
                              render: (text, record) => (
                                <>
                                  ...
                                  <Popconfirm
                                    title="Are you sure to delete Doctor visit？"
                                    placement="topRight"
                                    onConfirm={() => {
                                      handleRemove([record])
  ...
  useEffect(() => {
    queryDoctor_visitDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryDoctor_visitListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Input.Search style={{ marginRight: 20 }} placeholder="Search Doctor visit" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'Doctor visit-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryDoctor_visit({ ...params, sorter, filter })}
