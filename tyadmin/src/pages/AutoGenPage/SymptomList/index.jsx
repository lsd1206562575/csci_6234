...
import { addSymptom, querySymptom, removeSymptom, updateSymptom, querySymptomVerboseName, querySymptomListDisplay, querySymptomDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateSymptom(value, current_id);
      hide();
          ...
          width: 100,
                              render: (text, record) => (
                                <>
                                  ...
                                  <Popconfirm
                                    title="Are you sure to delete Symptom？"
                                    placement="topRight"
                                    onConfirm={() => {
                                      handleRemove([record])
  ...
  useEffect(() => {
    querySymptomDisplayOrder().then(r => {
  ...
  useEffect(() => {
    querySymptomListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, querySymptom, table_columns, 'Symptom-All')}>
            <ExportOutlined /> Export all
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder=" Search Symptom" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, 'Symptom-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => querySymptom({ ...params, sorter, filter })}
