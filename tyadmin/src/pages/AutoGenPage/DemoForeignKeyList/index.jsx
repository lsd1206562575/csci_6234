...
import { addDemoForeignKey, queryDemoForeignKey, removeDemoForeignKey, updateDemoForeignKey, queryDemoForeignKeyVerboseName, queryDemoForeignKeyListDisplay, queryDemoForeignKeyDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addDemoForeignKey({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateDemoForeignKey(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeDemoForeignKey(ids);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
  ...
  useEffect(() => {
    queryDemoForeignKeyDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryDemoForeignKeyListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryDemoForeignKey, table_columns, '全部字段非必填-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索全部字段非必填" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '全部字段非必填-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryDemoForeignKey({ ...params, sorter, filter })}
